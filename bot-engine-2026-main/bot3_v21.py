...
class Player(BaseBot):
    def __init__(self):
        ...
        # --- 1. DYNAMIC OPPONENT TRACKING (v18 BASE) ---
        self.total_hands = 0
        self.opp_raises = 0
        self.opp_folds = 0
        self.call_multiplier = 1.0  
    ...
    def on_hand_end(self, game_info: GameInfo, current_state: PokerState):
        if current_state.payoff > 0 and current_state.my_wager > current_state.opp_wager:
            self.opp_folds += 1
        if current_state.opp_wager > 40:
            self.opp_raises += 1

        if self.total_hands % 40 == 0 and self.total_hands > 0:
            aggression_rate = self.opp_raises / 40.0
            if aggression_rate > 0.45:
                self.call_multiplier = 0.80  # Opponent bluffs often -> soften our fold penalties
            elif aggression_rate < 0.15:
                self.call_multiplier = 1.20  # Opponent is tight -> respect their bets heavily
            else:
                self.call_multiplier = 1.00
    ...
    def get_p_win(self, current_state, game_info):
        ...
        # --- 2. ACCURATE 3-CARD BLINDSPOT TRACKING (v18 BASE) ---
        opp_card_count = 2
        # If the flop is dealt and we only have 2 cards, the opponent MUST have won the auction (3 cards).
        if current_state.street in ['flop', 'turn', 'river'] and len(current_state.my_hand) == 2:
            opp_card_count = 3
            
        opp_needed = max(0, opp_card_count - len(opp_locked))
    ...
    def get_move(self, game_info: GameInfo, current_state: PokerState):
        ...
        # --- 3. THE GT-BR AUCTION SHIFT (The v18 Killer) ---
        # v18 bids [75, 150] for strong, [20, 60] for marginal. 
        # We step directly over their brackets by exactly +1 to +14 chips.
        # This guarantees we win the auction against v18 without overpaying.
        if street == 'auction':
            if p_win > 0.85: 
                bid = random.uniform(0.8, 1.2) * min(pot * 2.5, 480) # Caps higher than v18's 450
            elif p_win > 0.65: 
                bid = random.uniform(151, 165) # Perfect step over v18's 150 max
            elif p_win > 0.50: 
                bid = random.uniform(61, 74)   # Perfect step over v18's 60 max
            else: 
                # Polarized bluff to beat their 0-15 and 150-300 ranges
                bid = random.uniform(301, 315) if random.random() < 0.12 else random.uniform(16, 19)
            
            return ActionBid(int(max(0, min(bid, my_chips))))

        # --- 4. BRUTAL MULTIPLICATIVE RANGE COLLAPSE (v18 BASE) ---
        pot_odds = cost / (pot + cost) if cost > 0 else 0
        adjusted_p_win = p_win

        if cost > 0:
            if not current_state.board:
                # Preflop collapse against massive shoves (The v12 Heuristic)
                if cost > 1500 and p_win < 0.82: adjusted_p_win *= 0.40 # Forces fold on AQ/JJ
                elif cost > 500 and p_win < 0.72: adjusted_p_win *= 0.60
                elif cost > 150 and p_win < 0.62: adjusted_p_win *= 0.80
            else:
                # Postflop collapse (Penetrates the 50% Pot Odds Illusion)
                pot_before_bet = max(1, pot - cost)
                bet_ratio = cost / pot_before_bet
                
                if bet_ratio > 1.5 and p_win < 0.94: adjusted_p_win *= 0.45 
                elif bet_ratio > 0.8 and p_win < 0.88: adjusted_p_win *= 0.65
                elif bet_ratio > 0.4 and p_win < 0.80: adjusted_p_win *= 0.85
                
            # --- THE BROKE FIX: RIVER RE-RAISE PENALTY ---
            # If it is the River, and WE already bet, and now they are RAISING us...
            # Their range is condensed entirely to the nuts. We fold all weak flushes/straights.
            if street == 'river' and my_wager > 0 and cost > 0:
                if p_win < 0.92:
                    adjusted_p_win *= 0.50 

            # Maniac Tracker Recovery: If they bluff heavily, we recover a fraction of our win-rate
            if self.call_multiplier < 1.0:
                recovery = (p_win - adjusted_p_win) * 0.35 # Max 35% recovery
                adjusted_p_win += recovery

        # --- 5. HARD EQUITY FLOORS (SUNK COST FIX) ---
        future_commitment = my_wager + cost
        if pot > 800 or future_commitment > 300:
            required_equity = max(pot_odds, 0.52)
            if (my_wager > 1000 or pot > 2000) and cost < (pot * 0.15):
                required_equity = pot_odds 
                
            if adjusted_p_win < required_equity:
                if cost > 0 and current_state.can_act(ActionFold): return ActionFold()
                return ActionCheck() if current_state.can_act(ActionCheck) else ActionCall()

        # Dynamic Preflop Defense (Stops bleeding to min-raises)
        if not current_state.board and cost > 0:
            required_preflop = 0.44 + min(0.12, (cost / 500.0) * 0.12)
            if adjusted_p_win < required_preflop:
                if current_state.can_act(ActionFold): return ActionFold()
                return ActionCheck() if current_state.can_act(ActionCheck) else ActionCall()

        # --- 6. DYNAMIC BETTING MATRIX (USES ADJUSTED_P_WIN) ---
        if current_state.can_act(ActionRaise):
            min_r, max_r = current_state.raise_bounds
            
            # --- THE SCREWDRIVERS EGO-WAR PREVENTER ---
            if not current_state.board and current_state.my_wager > 100:
                if p_win < 0.75: 
                    pass # Skip raising. Realize equity cheaply.
                else:
                    target = my_wager + cost + int(pot * random.uniform(0.8, 1.25))
                    return ActionRaise(int(max(min_r, min(target, max_r))))
                
            elif adjusted_p_win > 0.82:
                target = my_wager + cost + int(pot * random.uniform(0.8, 1.25))
                return ActionRaise(int(max(min_r, min(target, max_r))))
                
            elif adjusted_p_win > 0.65:
                target = my_wager + cost + int(pot * random.uniform(0.4, 0.65))
                return ActionRaise(int(max(min_r, min(target, max_r))))
                
            # --- THE SUICIDAL BLUFF PREVENTER ---
            elif adjusted_p_win < 0.45 and cost == 0 and pot < 300 and random.random() < 0.25:
                target = my_wager + cost + int(pot * random.uniform(0.3, 0.45))
                return ActionRaise(int(max(min_r, min(target, max_r))))

        # --- 7. EV DEFENSE (THE FINAL DECISION) ---
        if cost > 0:
            safety_margin = 0.02 if current_state.opp_revealed_cards else 0.08
            
            # --- THE FOLD_CLUB TRIPLE BARREL PENALTY ---
            pot_inflation_penalty = min(0.10, pot / 5000.0)
            total_margin = safety_margin + pot_inflation_penalty
            
            if adjusted_p_win > (pot_odds + total_margin):
