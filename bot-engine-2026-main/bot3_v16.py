...
class Player(BaseBot):
    def __init__(self):
        ...
        # Opponent Tracking for Dynamic Defense
        self.total_hands = 0
        self.large_bet_count = 0
    ...
    def on_hand_end(self, game_info: GameInfo, current_state: PokerState):
        self.total_hands += 1
        # Track opponent aggression. If they bet > 4x BB voluntarily, log it.
        # This allows us to statistically classify them as a "Maniac" or a "Nit".
        if current_state.opp_wager > 80: 
            self.large_bet_count += 1

    def get_p_win(self, current_state, game_info):
        """Information-Asymmetry Proxy Monte Carlo."""
        ...
        # Empirical Proxy Fix (Your Teammate's Logic): Post-flop, the opponent draws 3 cards 
        # to mathematically proxy their information advantage from Sneak Peek.
        opp_card_count = 3 if current_state.board else 2
    ...
    def get_move(self, game_info: GameInfo, current_state: PokerState):
        ...
        if street == 'auction':
            if p_win > 0.85:
                # Extreme Value: Bid high to secure the info (Caps at 400 safely)
                bid = random.uniform(0.8, 1.2) * min(pot * 2.0, 400)
            elif p_win > 0.65:
                # Mid Value: Steps squarely over "HELIOS" style bots that bid 60-70.
                bid = random.uniform(75, 150)
            elif p_win > 0.50:
                # Marginal
                bid = random.uniform(20, 60)
            else:
                # 15% Polarized Bluff Bid to confuse machine learning trackers
                bid = random.uniform(150, 300) if random.random() < 0.15 else random.uniform(0, 15)
            
            # The Kamikaze Trap Fix: Hard cap to 450 to never blow stack on single card.
            safe_cap = min(my_chips, 450)
            legal_bid = int(max(0, min(bid, safe_cap)))
            return ActionBid(legal_bid)

        # --- 2. LOCAL BEST RESPONSE (LBR) BAYESIAN NARROWING ---
        
        # Determine opponent archetype via frequency tracking
        maniac_ratio = self.large_bet_count / max(1, self.total_hands)
        
        # If they shove constantly (SluggishBays / ALL_IN_ALWAYS), base_penalty drops near zero.
        # If they only bet huge with the nuts (Tight bots), base_penalty rises to mathematically force folds.
        base_penalty = max(0.05, 0.5 - (maniac_ratio * 1.5))
        
        # Cost relative to the remaining pot size
        bet_size_ratio = cost / max(1, pot - cost)
        capped_ratio = min(bet_size_ratio, 2.5) # Bounds against math-breaking infinite shoves
        
        adjusted_p_win = p_win ** (1.0 + (capped_ratio * base_penalty))
        
        # --- 3. PREFLOP TAG (TIGHT-AGGRESSIVE) ARMOR ---
        future_commitment = my_wager + cost
        if not current_state.board:
            # If Maniac pushes pot above 350, demand a premium holding
            if future_commitment > 350 and adjusted_p_win < 0.60:
                return ActionFold() if current_state.can_act(ActionFold) else ActionCheck()
            # Standard Preflop Defense: Fold threshold is 0.44. (Protects KTs/QJs while folding garbage).
            elif cost > 0 and adjusted_p_win < 0.44:
                return ActionFold() if current_state.can_act(ActionFold) else ActionCheck()

        # --- 4. DYNAMIC BETTING MATRIX ---
        if current_state.can_act(ActionRaise):
            ...
            if p_win > 0.82:
                ...
                # Small-Ball Stab: Punish passive checkers.
                target = my_wager + cost + int(pot * random.uniform(0.3, 0.45))
                return ActionRaise(int(max(min_r, min(target, max_r))))

        # --- 5. EV DEFENSE ---
        if current_state.can_act(ActionCall):
            # Cost divided by the TOTAL pot POST-CALL gives true pot odds.
            # Example: 4950 bet into 50 pot -> Pot becomes 5000. Total post call pot = 9950. 
            # 4950 / 9950 = ~0.497 max pot odds.
