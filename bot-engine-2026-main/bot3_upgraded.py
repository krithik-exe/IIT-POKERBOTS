...
class Player(BaseBot):
    ...
    def get_preflop_equity(self, hand):
    ...
    # ============================
    # FAST POSTFLOP EQUITY APPROX
    # ============================

    def get_postflop_equity(self, hand, board):
        ...
        try:
            ...
            hand_type = eval7.handtype(my_val)

            strength = {
                'High Card': 0.20,
                'Pair': 0.50,
                'Two Pair': 0.70,
                'Three of a Kind': 0.80,
                'Straight': 0.90,
                'Flush': 0.93,
                'Full House': 0.97,
                'Four of a Kind': 0.99,
            ...
            return strength.get(hand_type, 0.20)
    ...
    def get_equity(self, state):
        ...
        if not state.board:
            return self.get_preflop_equity(state.my_hand)

        return self.get_postflop_equity(state.my_hand, state.board)
    ...
    def should_call(self, equity, pot, cost):
    ...
    def get_raise_size(self, equity, pot, min_raise, max_raise, stack):
        ...
        size = max(min_raise, min(size, max_raise))
    ...
    def auction_bid(self, equity, pot, stack):

        if equity > 0.75:
            return min(int(pot * 0.25), stack)

        elif equity > 0.60:
            return min(int(pot * 0.15), stack)

        elif equity > 0.50:
            return min(int(pot * 0.08), stack)
    ...
    def get_move(self, game_info: GameInfo, current_state: PokerState):
        ...
        equity = self.get_equity(current_state)
        ...
        if street == 'auction':
            ...
            bid = self.auction_bid(equity, pot, stack)

            return ActionBid(bid)


        # ============================
        # APPLY SNEAK PEEK PENALTY
        # ============================
        ...
        if current_state.can_act(ActionRaise):
            ...
            if equity > 0.60:
                ...
                raise_size = self.get_raise_size(
                    equity,
                    pot,
                    min_raise,
                    max_raise,
        ...
        if current_state.can_act(ActionCall):
            ...
            if self.should_call(equity, pot, cost):
        ...
        if current_state.can_act(ActionCheck):
            ...
            # bluff occasionally
            if equity < 0.40 and random.random() < 0.08:
